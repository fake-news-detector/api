...
use diesel::types::{Text, Integer, BigInt};
...
fn get_all_links(conn: DbConn) -> QueryResult<Json<Vec<LinkWithTopVote>>> {
    let query = sql::<(Integer, Text, Text, Integer, BigInt)>(
        "SELECT links.id, links.url, links.title, top_votes.category_id, top_votes.total
         FROM links
         INNER JOIN
